 ...
 * JLS 9, §13.4.3:
 *
 * <blockquote>If a class that was declared public is changed to not be declared
 * public, then an IllegalAccessError is thrown if a pre-existing binary is
 * linked that needs but no longer has access to the class type; such a change
 * is not recommended for widely distributed classes.</blockquote>
 */

@CImmutableStyleType
@VavrEncodingEnabled
@Value.Immutable
public interface CChangeClassBecameNonPublicType
  extends CChangeClassType
{
  ...
  default void checkPreconditions()
  {
    Preconditions.checkPrecondition(
      ...
      c -> "Class must not be non-public");

    Preconditions.checkPrecondition(
      this.classPrevious(),
      this.classPrevious().accessibility() == PUBLIC,
      c -> "Previous class must be public");
  }
