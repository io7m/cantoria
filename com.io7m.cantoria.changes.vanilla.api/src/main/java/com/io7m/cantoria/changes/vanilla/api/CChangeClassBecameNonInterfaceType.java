...
/**
 * @see CChangeClassBecameNonAbstractType
 *
 * Additionally, if an interface becomes any other type of class, this breaks
 * source compatibility with existing code and therefore should be considered a
 * backwards-incompatible change according to semantic versioning.
 */

@CImmutableStyleType
@VavrEncodingEnabled
@Value.Immutable
public interface CChangeClassBecameNonInterfaceType
  extends CChangeClassType
{
  ...
  default void checkPreconditions()
  {
    Preconditions.checkPrecondition(
      this.classValue(),
      !this.classValue().modifiers().contains(CModifier.INTERFACE),
      c -> "Class must not be an interface");

    Preconditions.checkPrecondition(
      this.classPrevious(),
      this.classPrevious().modifiers().contains(CModifier.INTERFACE),
      c -> "Previous class must be an interface");
  }
