...
/**
 * @see CChangeClassBecameAbstractType
 *
 * Additionally, if an abstract class becomes an interface, this breaks source
 * compatibility with existing code and therefore should be considered a
...
@VavrEncodingEnabled
@Value.Immutable
public interface CChangeClassBecameInterfaceType
  extends CChangeClassType
{
  ...
  default void checkPreconditions()
  {
    Preconditions.checkPrecondition(
      ...
      c -> "Class must be an interface");

    Preconditions.checkPrecondition(
      this.classPrevious(),
      !this.classPrevious().modifiers().contains(CModifier.INTERFACE),
      c -> "Previous class must not be an interface");
  }
