 ...
 * JLS 9 §13.4.17:
 *
 * <blockquote> Changing an instance method that is not declared final to be
 * declared final may break compatibility with existing binaries that depend on
 * the ability to override the method.</blockquote>
 *
 * This implementation considers making a method non-final to be a new feature
 * and therefore requires a minor version increment in terms of semantic
 * versioning.
 */
...
@VavrEncodingEnabled
@Value.Immutable
public interface CChangeClassMethodBecameFinalType
  extends CChangeMethodType
{
  ...
  default void checkPreconditions()
  {
    Preconditions.checkPrecondition(
      this.method().modifiers(),
      this.method().modifiers().contains(CModifier.FINAL),
      m -> "Modifiers must contain 'final'");

    Preconditions.checkPrecondition(
      this.methodPrevious().modifiers(),
      !this.methodPrevious().modifiers().contains(CModifier.FINAL),
